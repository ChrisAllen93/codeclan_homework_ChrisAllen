...
SELECT
	count(country) AS num_employees_in_spain_or_portugal
FROM employees 
...
SELECT
	count(*) AS num_missing_local_account_no
FROM pay_details
...
SELECT
	count(*) AS num_names_start_with_F
FROM employees 
...
SELECT 
	count(*) AS non_ger_fra_pension_enrolled
FROM employees 
WHERE pension_enrol = TRUE AND (country != 'France' OR country != 'Germany');

/*
 * Question 14 
 * What is the maximum salary among those employees in the ‘Engineering’ department who work 1.0
 * full-time equivalent hours (fte_hours)?
 */
 ...
 * style: ‘Bob Smith - Legal’. Restrict output to only those employees with stored first_name, last_name
 * and department.
 */
 ...
 * to the badge_label to celebrate long-standing colleagues, in the following style ‘Bob Smith - Legal
 * (joined 1998)’. Further restrict output to only those employees with a stored start_date.
 * 
